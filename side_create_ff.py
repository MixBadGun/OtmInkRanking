...
def SideVideo(main_end,side_end,side_count):
    ...
    # sideStaff = ffmpeg.filter(sideStaff,"scroll")
    ...
    for fournum in range(main_end+1,main_end+side_end+1,side_count):
        ...
        # sidePic = ffmpeg.filter(sidePic,"fade",t="in",d=0.25,alpha=1)
        # sidePic = ffmpeg.filter(sidePic,"fade",t="out",st=sideDur-0.25,d=0.25,a6lpha=1)
    ...
    ffmpeg.output(sideVideo,sideBGMAudio,"./output_clips/SideRank.mp4",vcodec="h264_nvenc",video_bitrate="10000k",audio_bitrate="320k").run()